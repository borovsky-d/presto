...
final class ApacheHiveTimestampDecoder
{
    ...
    {
        long millis = (seconds + options.baseTimestampInSeconds()) * options.timestampUnitsPerSecond();
        ...
        // (42)*1000 + 1 = 42001
        if (millis < 0 && nanos != 0) {
            millis -= options.timestampUnitsPerSecond();
        }
        // Truncate nanos to millis and add to mills
        return millis + (nanos / options.nanosecondsPerTimestampUnit());
    }
